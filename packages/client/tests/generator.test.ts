...
import { buildArgs, buildImpl, buildInterface, buildParams, indent } from "../scripts/lib/build";
...
describe("Builder - params & args generation", () => {
  ...
  it("flattens parameters/args when flatten is true and only has body", () => {
    ...
    expect(buildParams(method)).toBe(
      `body: ${method.typeSignature}["requestBody"]["content"]["application/json"]`,
    );
...
describe("Builder - buildInterface & buildImpl", () => {
  ...
  it("generates typescript interface definition with doc example", () => {
    ...
    expect(code).toContain(
      'getUser(params: paths["/users/{id}"]["get"]["parameters"]["path"]): Promise<any>;',
    );
  });
