...
export function buildParams(m: GeneratedMethod): string {
  ...
  const parts: string[] = [];
  if (m.hasBody) {
    parts.push(`body: ${m.typeSignature}["requestBody"]["content"]["application/json"]`);
  }
  if (m.hasPathParams || m.hasQueryParams) {
    const paramParts: string[] = [];
    if (m.hasPathParams) paramParts.push(`path: ${m.typeSignature}["parameters"]["path"]`);
    if (m.hasQueryParams) paramParts.push(`query: ${m.typeSignature}["parameters"]["query"]`);
